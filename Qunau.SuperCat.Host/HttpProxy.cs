    ...
    public static class HttpProxy
    {
        ...
        public static void Start()
        {
            ThreadPool.QueueUserWorkItem(o =>
            {
                ...
                listener.Prefixes.Add(string.Format("http://{0}:{1}/", Config.ProxyAddress, Config.ProxyPort));
                listener.Start();
                while (true)
                {
                    try
                    {
                        ...
                        ThreadPool.QueueUserWorkItem((m) =>
                        {
                            try
                            {
                                Process(m as HttpListenerContext);
                            }
                            catch (Exception ex)
                            {
                                Qunau.NetFrameWork.Common.Write.LogService.SaveLog("代理处理异常", "request", ex);
                            }
                        }, request);
                    }
                    catch (Exception ex)
        ...
        private static string GetPortFromQueryString(HttpListenerContext context)
        {
        ...
        private static ContentType GetContentType(string txt, bool isHtml = false)
        {
        ...
        /// <returns></returns>
        public static byte[] ReplaceAndAppend(byte[] buffer, string contentType, string url)
        {
        ...
        /// <returns></returns>
        private static byte[] Decompress(byte[] buffer, string contentEncoding)
        {
        ...
        private static void SetCookies(HttpWebRequest request, HttpListenerContext context)
        {
            try
            {
